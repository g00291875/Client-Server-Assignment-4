...
public class TicTacToeServer extends JFrame {
   ...
   private class Player extends Thread {
               ...
               try{
                  ...
                  if(! isGameOver())
                     if (validateAndMove(location, playerNumber)) { // returns true if it's a valid move
                        //displayMessage("\nlocation: " + location); // output to server terminal
                        output.writeUTF("Valid move."); // output to current thread
      ...
      public synchronized void outputWhoWon(){
         try {
            int winner = (currentPlayer == PLAYER_X ? 0 : 1 );
               switch (winner) {
                  case 0:                      // PLayer X wins
                     players[1].output.writeUTF("you win");
                     break;
                  case 1:                      // PLayer Y wins
                     players[0].output.writeUTF("you win");
                     break;
            }
...
//            }
      ...
      public int checkWin(int location){
         if(playerNumber == 0) {
            if (board[5] == X_MARK)
               return 1;
         }
