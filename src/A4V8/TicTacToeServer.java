...
public class TicTacToeServer extends JFrame {
   ...
   private class Player extends Thread {
         ...
         try {/*************************************************************START RUN ***/
            ...
            while (! isGameOver() ) {          /***** MAIN LOOP MAIN LOOP MAIN LOOP MAIN LOOP MAIN LOOP **/
               try{
                  ...
                  if (validateAndMove(location, playerNumber) && ! isGameOver()) { // returns true if it's a valid move
      ...
      public void outputWhoWon(){
          synchronized(this) {
              try {
                   winLossDraw = (this.playerNumber == PLAYER_X ? winLossDraw++ : winLossDraw+2 );
                 // winLossDraw = (playerNumber == 0 ? winLossDraw++ : winLossDraw + 2);
                  switch (winLossDraw) {
                      case 0:                      // PLayer Y wins
                          output.writeUTF("you draw");
                          output.writeUTF("you draw");
                          break;
                      case 1:                      // PLayer X wins
                          output.writeUTF("you win as player o");
                         // players[1].output.writeUTF("you lose as player o");
                          break;
                      case 2:                      // PLayer Y wins
                            players[0].output.writeUTF("you lose as player x");
                            output.writeUTF("you win as player x");
                          break;
                      case 3:                      // PLayer X wins
                          //output.writeUTF("you win as player x");
                          output.writeUTF("you lose as player o");
                          break;
                      case 4:                      // PLayer Y wins
                          output.writeUTF("you lose as player x");
                          //players[1].output.writeUTF("you win as player o");
                          break;
                      case 5:                      // PLayer Y wins
                          players[0].output.writeUTF("you draw");
                          players[1].output.writeUTF("you draw");
                          break;
                      default:
                          break;
                  }
      ...
      public synchronized int checkWin(int location){
         if(playerNumber == 0) {
            if (board[5] == X_MARK)
               return 1;
         }
